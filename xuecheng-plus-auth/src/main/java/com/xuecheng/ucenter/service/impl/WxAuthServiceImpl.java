...
public class WxAuthServiceImpl implements AuthService, WxAuthService {
    ...
    @Override
    public XcUserExt execute(AuthParamsDto authParamsDto) {  // 这个不是首次使用微信登录，所以可以不需要校验了吗？
        //得到账号
        String username = authParamsDto.getUsername();
                                                        // 不需要判断验证码
        //查询数据库
        XcUser xcUser = xcUserMapper.selectOne(new LambdaQueryWrapper<XcUser>().eq(XcUser::getUsername, username)); //  只需要查是否存在该用户
        if(xcUser == null){
            throw new RuntimeException("用户不存在");
        }

        XcUserExt xcUserExt = new XcUserExt();    // 不需要比对密码
