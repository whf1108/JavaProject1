...
//            configuration.setDirectoryForTemplateLoading(new File(classpath+"/templates/"));
    ...
    public void uploadCourseHtml(Long courseId, File file) {
        try {
            //将file转成MultipartFile，feign远程调用需要配置
            MultipartFile multipartFile = MultipartSupportConfig.getMultipartFile(file);
            //远程调用得到返回值
            String upload = mediaServiceClient.upload(multipartFile, "course/" + courseId + ".html"); // 修改为String类型
            if (upload == null) {
...
//                return coursePublish;
//            }else{
//                synchronized (this){   // 这个目的是第一次高并发来了，缓存没有数据，肯定很多线程进来。缓存穿透也会有这个现象啊！！！和缓存击穿没啥关系吧
//                    //再次查询一下缓存
//                    //从缓存中查询
...
//                }
                //从数据库查询
                CoursePublish coursePublish = getCoursePublish(courseId);    // 就算course是null，也设置缓存
                //查询完成再存储到redis
                redisTemplate.opsForValue().set("course:" + courseId, JSON.toJSONString(coursePublish), 300, TimeUnit.SECONDS);
                return coursePublish;
