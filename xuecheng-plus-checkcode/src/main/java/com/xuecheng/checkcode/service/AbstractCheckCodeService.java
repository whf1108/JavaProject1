...
public abstract class AbstractCheckCodeService implements CheckCodeService {
    ...
    protected CheckCodeGenerator checkCodeGenerator;  // 接口里面定义的函数已经被重写了
    protected KeyGenerator keyGenerator;   // 接口里面定义的函数已经被重写了
    protected CheckCodeStore checkCodeStore;   // 这个是只哪个存储（内存还是redis）
    ...
    */
    public GenerateResult generate(CheckCodeParamsDto checkCodeParamsDto,Integer code_length,String keyPrefix,Integer expire){
        ...
        //生成一个key
        String key = keyGenerator.generate(keyPrefix);  // 前缀
    ...
    // CheckCodeResultDto： key+混淆后的内容aliasing
